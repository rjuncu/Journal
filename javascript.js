function store(){
  ...
  if (localStorage.getItem("posts") !== null){  
    ...
    let prevPosts = JSON.parse(localStorage.posts);
    console.log(prevPosts.push(post1));
...
function displayPosts() {
  let post1 = (JSON.parse(localStorage.posts));
  console.log(post1.title);
  document.getElementById("stage").innerHTML = '<div class="card text-left"><div class="card-body"><h5 class="card-title">'+post1.title+'</h5><p class="card-text">'+post1.body+'</p></div><div class="card-footer text-muted">'+post1.date+'</div></div>';
